...
export function NicknamesModule() {
    minecraft.system.runInterval((e) => {
        for (const player of minecraft.world.getPlayers()) {
            ...
            player.nameTag = `${nameColor}${
                player.hasTag("name-bold") ? "§l" : ""
            }${player.nameTag
                .split("\n")[0]
                .replace(
                    /\u00A7[0-9a-gk-orA-GKM-OR]/gi,
                    ""
                )}\n§r${bracketColor}< ${
                rankText ? rankText : "§dMember"
            } §r${bracketColor}>\n§2${healthComponent.current}/${
                healthComponent.value
            } §a${
                "█".repeat(
                    Math.floor(
                        (healthComponent.current / healthComponent.value) * 5
                    )
                ) +
                "░".repeat(
                    5 -
                        Math.floor(
                            (healthComponent.current / healthComponent.value) *
                                5
                        )
                )
            }`;
