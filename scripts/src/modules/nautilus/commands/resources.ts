...
export const NautilusResourcesCommand = {
    ...
    exec(msg, args, commands, toggles, azaleaEnv) {
        if(args.length && args[0] == "manage") return function(chatmsg) {
            if(!chatmsg.sender.isOp()) return true;
            if(!chatmsg) {
                minecraft.world.sendMessage("§aWelcome to Nautilus Resources management menu. Type §2exit §ato exit");
                return false;
    
            }
            if(chatmsg.message == "exit") return true;
            switch(chatmsg.message.toLowerCase()) {
                case "get info":
                    function get_info(message) {
                        console.warn("GET: INFO")
                        let db = new Database(120); // 120 is nautilus table ID
                        let serverInfo:string = db.get("Resources.ServerInfo");
                        let rules:string = db.get("Resources.Rules");
                        console.warn(JSON.stringify(serverInfo))
                        console.warn(JSON.stringify(rules));
                        let returnMessage = [];
                        if(!serverInfo.length) returnMessage.push("§cNo server info set! Type §4server info name §cor §4server info description §cin chat");
                        if(!rules.length) returnMessage.push("§cNo rules! Type §4add rule§c, §4get rule§c, or §4remove rule §cin chat");
                        if(serverInfo) {
