...
system.events.scriptEventReceive.subscribe(({ sourceEntity, message, id }) => {
    if (!(sourceEntity instanceof Player))
        return;
    const table = message.split(" ")[0];
    ...
    switch (id) {
        case "database:set":
            ...
            sourceEntity.tell(`Set Key: "${key}", to value: "${value}" on table: "${table}"`);
            break;
        case "database:get":
        ...
        case "database:strain":
            ...
            system.events.beforeWatchdogTerminate.subscribe((data) => {
                data.cancel = true;
                sourceEntity.tell(`§cStrain Failed at: ${~~((Date.now() - startTime) / 1000)} Seconds`);
            });
            ...
            sourceEntity.tell(`§aCompleted strain in: ${~~((Date.now() - startTime) /
                1000)} Seconds`);
            break;
        default:
            break;
