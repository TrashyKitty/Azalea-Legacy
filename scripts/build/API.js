...
export class PlayerAPI {
    ...
    teleport(player = this.player, location, dimension, sound = true) {
        player.teleport(this.possibleArrayToVector3(location), minecraft.world.getDimension(dimension), player.getRotation().x, player.getRotation().y);
        if (sound) {
        }
    }
