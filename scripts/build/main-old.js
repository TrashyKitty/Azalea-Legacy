...
function setToggle(name, bool) {
    ...
    minecraft.world
        .getDimension("overworld")
        .runCommandAsync(`scoreboard players set "${name}" toggles ${bool ? "1" : "0"}`)
        .then(getToggles);
}
let leaderboards = [];
        ...
        try {
            if (false) {
                ...
                if (createEntity) {
                    ...
                    let entity = overworld.spawnEntity("minecraft:rabbit", new minecraft.Location(leaderboard.pos[0], leaderboard.pos[1], leaderboard.pos[2]));
                ...
                if (leaderboard.objective != "online") {
                    const objective = minecraft.world.scoreboard.getObjective(leaderboard.objective);
                    const participants = objective.getParticipants();
                    ...
                    for (const score of scores) {
                        if (score[0].displayName == "commands.scoreboard.players.offlinePlayerName")
                            continue;
...
minecraft.world.events.beforeItemUse.subscribe((event) => {
    if (event.item.typeId == "minecraft:echo_shard" &&
        ...
        const uiMain = new ui.ActionFormData()
            ...
            if (result.selection == 2) {
                const configUI2 = new ui.ModalFormData()
                    ...
                    .show(player)
                    .then((res) => {
                    player.nameTag = `§${res.formValues[0].toString(16)}${player.name}`;
                });
            }
        });
    }
...
minecraft.world.events.beforeChat.subscribe((msg) => {
    msg.cancel = true;
    getToggles();
    if (azaleaEnv.AlternativeMessageHandler)
        azaleaEnv.AlternativeMessageHandler(msg, commands, toggles, azaleaEnv, playerCommandModeMap, getToggles, DEVELOPERS);
    if (msg.message.startsWith("!")) {
        if (azaleaEnv.alternativeMessageHandlerModifiesCommandHandler)
            return;
    ...
    if (azaleaEnv.alternativeMessageHandlerModifiesMessageHandler)
        return;
